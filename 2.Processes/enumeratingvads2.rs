    ...
    unsafe{
        ...
        if rootnode.left!=0{
            ...
            if tag==[86u8,97 ,100,32,0] {
                ...
                //let filepointer = filepointer & 0xFFFFFFFF_FFFFFFF0;
                // filepointer + 0x58 gives unicode_String of module
                /*if filepointer !=0 && controlarea!=0 && subsection!=0{
                    let filepointer = filepointer & 0xFFFFFFFF_FFFFFFF0;
                    // filepointer + 0x58 gives unicode_String of module
                    let us = core::ptr::read((filepointer+0x58) as *const UNICODE_STRING);
                    if us.Length!=0 && us.MaximumLength!=0{
                        ...
                        /*let mut founddll = 1;
                        // \Windows\System32\ntdll.dll
                        // 92 87 105 110 100 111 119 115 92 83 121 115 116 101 109 51 50 92 110 116 100 108 108 46 100 108 108
                        let dllnamestring = core::str::from_utf8(&dllname);
                        if dllnamestring.is_ok(){
                            if dllnamestring.unwrap().contains("ntdll.dll"){
                                founddll=1;
                            }
                            else{
                                founddll=0;
...
pub extern "system" fn driver_entry(_driver: &mut DRIVER_OBJECT,
    ...
    unsafe {
        ...
        let mut eprocess:u64 = 0;
        PsLookupProcessByProcessId(8588 as HANDLE, &mut eprocess as *mut _ as *mut c_void);
