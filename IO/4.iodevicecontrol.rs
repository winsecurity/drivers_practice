...
use windows_kernel_sys::ntoskrnl::MmGetSystemAddressForMdlSafe;
...
pub extern "system" fn iocreateclose(deviceobject: &mut DEVICE_OBJECT,irp: &mut IRP) -> i32{
    unsafe{
        ...
        core::ptr::write((irp as *mut _ as usize+0x30+0x8) as *mut u64, 0);
...
#[no_mangle]
pub extern "system" fn ioread(deviceobject: &mut DEVICE_OBJECT,irp: &mut IRP) -> i32{
    unsafe{
        ...
        let userbuffer = core::ptr::read((irp as *const _ as usize +0x70) as *const u64);
        for i in 0..nbytes{
            core::ptr::write((userbuffer+i) as *mut u8, 0x61);
        }
        // BUFFERED IO
        //let sysbuffer = core::ptr::read((irp as *mut _ as usize+0x18) as *const u64);

        //core::ptr::write(sysbuffer as *mut [u8;3], [0x61u8;3]);
...
#[no_mangle]
pub extern "system" fn iodevicecontrol(deviceobject: &mut DEVICE_OBJECT,irp: &mut IRP) -> i32{
    unsafe{

        let piostack = IoGetCurrentIrpStackLocation(irp);

        let mut outputbufferlength=0;
        let mut inputbufferlength=0;
        if !piostack.is_null(){
            outputbufferlength = core::ptr::read((piostack as *mut _ as usize+0x8) as *const u64);
            inputbufferlength = core::ptr::read((piostack as *mut _ as usize+0x8+0x8) as *const u64);

        }

         // Associated irp contains the inputbuffer address.
         let inputbuffer = core::ptr::read((irp as *const _ as usize+0x18) as *const u64);
        DbgPrint("inputbuffer string: %s\n\0".as_ptr(),inputbuffer as *const u8);
       

        // reading mdladdress - it contains outputbuffer address we need to write
        let mdladdress = core::ptr::read((irp as *const _ as usize+0x8) as *const u64);
        let systemva = MmGetSystemAddressForMdlSafe(
            core::mem::transmute(mdladdress ), 0);

        if !systemva.is_null(){
            for i in 0..outputbufferlength{
                core::ptr::write((systemva as usize+i as usize) as *mut u8, 0x66);

            }
            //DbgPrint("inputbuffer: %s\n\0".as_ptr(),systemva as c_ulonglong);
        }
...
pub extern "system" fn driver_entry(driverobject: &mut DRIVER_OBJECT,
    ...
    unsafe {
       ...
       driverobject.DriverUnload = core::mem::transmute(driver_unload as *mut c_void);
        ...
        let res = IoCreateDevice(driverobject, 0, 
            &devicename, 
            0x00000022, 
        ...
        if res==STATUS_SUCCESS{
            ...
            if res2==STATUS_SUCCESS{
                ...
                driverobject.MajorFunction[IRPMJ::IRP_MJ_CREATE as usize] = core::mem::transmute(iocreateclose as *mut c_void);
                driverobject.MajorFunction[IRPMJ::IRP_MJ_CLOSE as usize] = core::mem::transmute(iocreateclose as *mut c_void);
                driverobject.MajorFunction[IRPMJ::IRP_MJ_WRITE as usize] = core::mem::transmute(iowrite as *mut c_void);
                driverobject.MajorFunction[IRPMJ::IRP_MJ_READ as usize] = core::mem::transmute(ioread as *mut c_void);
                driverobject.MajorFunction[IRPMJ::IRP_MJ_DEVICE_CONTROL as usize] =  core::mem::transmute(iodevicecontrol as *mut c_void);
            }
...
#[no_mangle]
pub extern "system" fn driver_unload(driverobject:&mut DRIVER_OBJECT){
    unsafe{
